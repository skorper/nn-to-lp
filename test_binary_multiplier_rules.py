...
Return SUM, CARRY
'''
def __full_adder(a, b, c_in):
	prefix = b[1:]
	print ("prefix: " + str(prefix))
	rules.append(("i" + prefix, a + " XOR " + b))
	rules.append(("s" + prefix, "i" + str(prefix) + " XOR " + c_in))
	rules.append(("c" + prefix, a + " AND " + b + " OR ( " + c_in + " AND " + str("i" + str(prefix)) + ")"))
	return str("s" + prefix), str("c" + prefix)

# TODO c is not correct
def add(num1, num2):
	...
	for i in range(len(num1) - 1, -1, -1):
		s, c = __full_adder(str(num1[i]), str(num2[i]), c)
		sum[i+1] = s
	sum[0] = c
	print ("sum: " + str(sum))
	return sum
...
def multiply(num1, num2):
	...
	for bit_2, i in zip(num2, range(len(num2))):
		...
		for bit_1, j in zip(num1, range(len(num1))):
			...
			temp[j] = "t" + str(__generate_id())
			if i == 0:
				sum[j] = "a" + str(__generate_id())
				rules.append((sum[j], "0"))
			rules.append((temp[j], bit_1 + " AND " + bit_2))
		for k in range(shift_index):
			suffix = str(__generate_id())
